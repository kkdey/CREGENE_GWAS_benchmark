## Generate bedgraph files from a gene score file and CRE-gene links files
...
for(numl in 1:length(ll)){
  ...
  out1 = ABC_all_calc(scores,
                      ...
                      output_bed = paste0("ABC_all.bed"))

  out2 = ChromHMM_all_calc(scores,
                           ...
                           output_bed = paste0("ChromHMM_all.bed"))

  out3 = EpiMap_all_calc(scores,
                         pred_file = snakemake@params$pred_file_EpiMap,
                         key_file = snakemake@params$key_file_EpiMap,
                         output_cell = paste0(snakemake@output$outdir, "/", annot_name),
                         output_bed = paste0("EpiMap_all.bed"))
}
